          ...
          <>
            ...
            <div className="list-section">
              ...
              {registryTab === 'matches' && (getMatches().length === 0 ? (<div style={{ textAlign: 'center', padding: '40px', color: '#888' }}>Активных матчей пока нет</div>) : (getMatches().map(m => (
                    <div className="match-card" key={m.id}><div className="match-badge">МАТЧ: {m.object.role === 'Продавец' ? 'ПРОДАЖА' : 'АРЕНДА'}</div><div className="match-split"><div className="match-side"><h4>ОБЪЕКТ ({m.object.role})</h4><p className="m-type">{m.object.type}</p><p className="m-info">{m.object.rooms} комн • {m.object.area} м² • Этаж {m.object.floor}</p><p className="m-dist">{m.object.district === 'Пропустить' ? 'Все районы' : m.object.district}</p><p className="m-price">{formatNumber(m.object.price)} ₽</p><span className="m-agent">Автор: {m.object.agent}</span></div><div className="match-divider"></div><div className="match-side"><h4>КЛИЕНТ ({m.client.role || 'Покупатель'})</h4><p className="m-type">Поиск: {m.client.propertytype || m.client.propertyType}</p><p className="m-info">{(m.client.roomsfrom || m.client.roomsFrom) || 0}-{(m.client.roomsto || m.client.roomsTo) || 0} комн • {(m.client.areafrom || m.client.areaFrom) || 0}-{(m.client.areato || m.client.areaTo) || 0} м² • Этаж: {(m.client.floorfrom || m.client.floorFrom) || 0}-{(m.client.floorto || m.client.floorTo) || 0}</p><p className="m-dist">{m.client.district === 'Пропустить' ? 'Все районы' : m.client.district}</p><p className="m-price">
  {(m.client.budgetfrom || m.client.budgetFrom)
    && !(m.client.budgetto || m.client.budgetTo)
      ? `От ${formatNumber(m.client.budgetfrom || m.client.budgetFrom)} ₽`
      : `До ${formatNumber(m.client.budgetto || m.client.budgetTo)} ₽`
  }
</p><span className="m-agent">Автор: {m.client.agent}</span></div></div></div>
                  ))))}
            </div>
          </>
        )}
