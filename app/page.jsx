...
export default function HomePage() {
  ...
  // --- ИСПРАВЛЕННАЯ ЛОГИКА ВХОДА (Вход или Регистрация) ---
  ...
  const addObject = async () => {
    ...
    if (!error) { fetchObjects(); setNewObject({ type: 'Квартира', price: '', rooms: '', area: '', floor: '', district: 'Ленинский', address: '' }); alert("Объект добавлен") }
  }

  const addClient = async () => {
    const { error } = await supabase.from('clients').insert([{ ...newClient, agent: agentName }])
    if (!error) { fetchClients(); setNewClient({ propertyType: 'Квартира', budgetFrom: '', budgetTo: '', roomsFrom: '', roomsTo: '', floorFrom: '', floorTo: '', areaFrom: '', areaTo: '', district: 'Ленинский', address: '' }); alert("Заявка добавлена") }
  }
    ...
    return (
      <main className="login-page">
        <div className="login-card">
          <h1>B2B GARANT</h1>
          <input className="form-input" placeholder="Имя агента" value={agentName} onChange={e => setAgentName(e.target.value)} />
          <input className="form-input" placeholder="+7 999 000 00 00" value={agentPhone} onChange={e => setAgentPhone(formatPhoneNumber(e.target.value))} />
          <button className="save-btn" onClick={handleLogin} disabled={loading}>{loading ? "ЗАГРУЗКА..." : "ВОЙТИ"}</button>
        </div>
      </main>
    )
  }
  ...
  return (
    <main className="crm-container">
      <header className="topbar"><h1>B2B GARANT</h1><button className="profile-btn"><User size={20} /></button></header>
      <section className="content" style={{ paddingBottom: '100px' }}>
            ...
            <div className="agents-section">
              ...
              {allAgents.map((agent, index) => (
                <div key={agent.id} className="agent-rank-card">
                  ...
                  <div style={{ flex: 1 }}><h3>{agent.name}</h3><p>{agent.phone}</p></div>
                  <Medal size={20} />
                </div>
              ))}
            </div>
        ...
        {activeTab === 'registry' && (
          <>
            ...
            {registryTab === 'objects' && objects.map(o => <div className="registry-card" key={o.id}><h3>{o.type}</h3><p>{o.address}</p><strong>{formatNumber(o.price)} ₽</strong></div>)}
            {registryTab === 'clients' && clients.map(c => <div className="registry-card" key={c.id}><h3>{c.propertyType}</h3><p>Бюджет: {formatNumber(c.budgetFrom)}</p></div>)}
            {registryTab === 'agents' && allAgents.map(a => <div className="registry-card" key={a.id}><h3>{a.name}</h3><p>{a.phone}</p></div>)}
          </>
        )}
      </section>
      ...
      <style jsx>{`
        ...
        .bottom-nav button { background: none; border: none; display: flex; flex-direction: column; align-items: center; color: #ccc; }
        .bottom-nav button.active { color: #000; }
        .form-input { padding: 12px; border-radius: 8px; border: 1px solid #ddd; width: 100%; margin-bottom: 10px; }
        .save-btn { background: #000; color: #fff; padding: 15px; border-radius: 8px; width: 100%; border: none; font-weight: bold; }
        .stat-box-simple { border: 1px solid #eee; padding: 10px; border-radius: 12px; text-align: center; }
